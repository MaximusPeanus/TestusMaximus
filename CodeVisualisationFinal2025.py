...
# Configuration du port série PC test
PORT_UTILISE = 'COM4'  # Port série utilisé
...
class FenetrePrincipale(QMainWindow):
    def __init__(self):
        ...
        self.donnees = {station: {variable: deque(maxlen=None) for variable in variables} for station, variables in self.stations.items()}
    ...
    def maj_graphique(self):
        ...
        has_data = False
        all_times = []  # Nouveau: liste pour stocker tous les timestamps
            ...
            for variable, action in variables.items():
                if action.isChecked():
                    if len(self.donnees[station][variable]) > 0:
                        # Modification ici: récupération des temps
                        ...
                        all_times.extend(temps)  # Nouveau: collecte des temps
                        has_data = True
        ...
        if not has_data:
            ...
            # Nouveau code pour l'adaptation de l'axe X
            min_time = min(all_times) if all_times else datetime.now()
            max_time = max(all_times) if all_times else datetime.now()
        
            # Ajout d'une marge de 5% 
            time_range = max_time - min_time
            margin = time_range.total_seconds() * 0.05
            self.ax.set_xlim(
                min_time - dt.timedelta(seconds=margin),
                max_time + dt.timedelta(seconds=margin)
            )
